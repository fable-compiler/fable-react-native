...
module ImageResizer =
    type ResizerResult() =
        member this.Uri : string = this?uri |> unbox
    ...
    let createResizedImage (path: string, maxWidth: int, maxHeight: int, compressFormat: string, quality: int) : JS.Promise<ResizerResult> =
        ReactNativeImageResizer.Globals.ImageResizer?createResizedImage(path, maxWidth, maxHeight, compressFormat, quality)
        |> unbox
